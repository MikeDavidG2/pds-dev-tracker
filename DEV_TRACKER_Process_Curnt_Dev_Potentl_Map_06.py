...
# Purpose:
"""
To update the CURRENT Development Potential (Map 06) data in SDE by
...
We are first finding the predicted available DU per HEX (or CPASG) by:
    Map 01 + Map 02 + Map 12 (General Plan Allowed + Approved GPAs + Annexations/Acquisitions/Programs)
We are then subtracting from those predicted values the sum of:
    Map 03 + Map 04 (Existing DU + Approved DU)

(Map 01 + Map 02 + Map 12) - (Map 03 + Map 04)

The end result is a HEXBIN FC and a CPASG Table that has the values of the number
of DU that can still be added to each HEXBIN or CPASG (WITHOUT taking into account
any "In-Process" DU's)
"""
#
# Author:      mgrue
...
def main():
    ...
    shorthand_name    = 'Curnt_Dev_Potentl_Map_06'
    ...
    gp_allowed_map_01_new_fld         = 'GP_Allowed_Map_01'
    approved_gpa_map_02_new_fld       = 'Approved_GPA_Map_02'
    annex_aquitns_map_12_new_fld      = 'Annex_Aquitns_Map_12'
    existing_du_map_03_new_fld        = 'Existing_DU_Map_03'
    approved_du_map_04_new_fld        = 'Approved_DU_Map_04'
    curnt_dev_potentl_map_06_new_fld  = 'VALUE_Curnt_Dev_Potentl_Map_06'
    ...
    if success == True:
        try:
            ...
            field_type = 'DOUBLE'
            fields_to_add = [gp_allowed_map_01_new_fld,
                             approved_gpa_map_02_new_fld,
                             annex_aquitns_map_12_new_fld,
                             existing_du_map_03_new_fld,
                             approved_du_map_04_new_fld,
                             curnt_dev_potentl_map_06_new_fld]
            ...
            #          Get the values in each hexbin from Map 01
            print('\n---------------------------------------------------------')
            print('Getting the values in each hexbin from Map 01:')

            Join_and_Calc_Fields_HEX(hexbin_fc, hexbin_id_exst_fld, gp_allowed_map_01_hex_fc, gp_allowed_map_01_exst_hex_fld, gp_allowed_map_01_new_fld)


            #-------------------------------------------------------------------
            #          Get the values in each hexbin from Map 02
            print('\n---------------------------------------------------------')
            print('Getting the values in each hexbin from Map 02:')

            Join_and_Calc_Fields_HEX(hexbin_fc, hexbin_id_exst_fld, approved_gpa_map_02_hex_fc, approved_gpa_map_02_exst_hex_fld, approved_gpa_map_02_new_fld)


            #-------------------------------------------------------------------
            #          Get the values in each hexbin from Map 12
            print('\n---------------------------------------------------------')
            print('Getting the values in each hexbin from Map 12:')

            Join_and_Calc_Fields_HEX(hexbin_fc, hexbin_id_exst_fld, annex_aquitns_map_12_hex_fc, annex_aquitns_map_12_exst_hex_fld, annex_aquitns_map_12_new_fld)


            #-------------------------------------------------------------------
            #          Get the values in each hexbin from Map 03
            print('\n---------------------------------------------------------')
            print('Getting the values in each hexbin from Map 03:')

            Join_and_Calc_Fields_HEX(hexbin_fc, hexbin_id_exst_fld, existing_du_map_03_hex_fc, existing_du_map_03_exst_hex_fld, existing_du_map_03_new_fld)


            #-------------------------------------------------------------------
            #          Get the values in each hexbin from Map 04
            print('\n---------------------------------------------------------')
            print('Getting the values in each hexbin from Map 04:')

            Join_and_Calc_Fields_HEX(hexbin_fc, hexbin_id_exst_fld, approved_du_map_04_hex_fc, approved_du_map_04_exst_hex_fld, approved_du_map_04_new_fld)


            #-------------------------------------------------------------------
            #              Calculate the VALUE field for Map 06
            print('\n\n-------------------------------------------------------')

            # Set the expression for the calculation
            expression = '(!{map_01}!+!{map_02}!+!{map_12}!)-(!{map_03}!+!{map_04}!)'.format(map_01=gp_allowed_map_01_new_fld,
                                                                                             map_02=approved_gpa_map_02_new_fld,
                                                                                             map_12=annex_aquitns_map_12_new_fld,
                                                                                             map_03=existing_du_map_03_new_fld,
                                                                                             map_04=approved_du_map_04_new_fld)


            # Calculate field
            print '\n  Calculating field [{}] to equal: "{}"\n'.format(curnt_dev_potentl_map_06_new_fld, expression)
            arcpy.CalculateField_management(hexbin_fc, curnt_dev_potentl_map_06_new_fld, expression, 'PYTHON_9.3')
    ...
    if success == True:
        try:
            ...
            #          Get the values in each CPASG from Map 01
            print('\n---------------------------------------------------------')
            print('Getting the values in each CPASG from Map 01:')

            Join_and_Calc_Fields_CPASG(cpasg_tbl, cmty_plan_id_exst_fld, gp_allowed_map_01_cpasg_tbl, gp_allowed_map_01_exst_cpasg_fld, gp_allowed_map_01_new_fld)


            #-------------------------------------------------------------------
            #          Get the values in each CPASG from Map 02
            print('\n---------------------------------------------------------')
            print('Getting the values in each CPASG from Map 02:')

            Join_and_Calc_Fields_CPASG(cpasg_tbl, cmty_plan_id_exst_fld, approved_gpa_map_02_cpasg_tbl, approved_gpa_map_02_exst_cpasg_fld, approved_gpa_map_02_new_fld)


            #-------------------------------------------------------------------
            #          Get the values in each CPASG from Map 12
            print('\n---------------------------------------------------------')
            print('Getting the values in each CPASG from Map 12:')

            Join_and_Calc_Fields_CPASG(cpasg_tbl, cmty_plan_id_exst_fld, annex_aquitns_map_12_cpasg_tbl, annex_aquitns_map_12_exst_cpasg_fld, annex_aquitns_map_12_new_fld)


            #-------------------------------------------------------------------
            #          Get the values in each CPASG from Map 03
            print('\n---------------------------------------------------------')
            print('Getting the values in each CPASG from Map 03:')

            Join_and_Calc_Fields_CPASG(cpasg_tbl, cmty_plan_id_exst_fld, existing_du_map_03_cpasg_tbl, existing_du_map_03_exst_cpasg_fld, existing_du_map_03_new_fld)


            #-------------------------------------------------------------------
            #          Get the values in each CPASG from Map 04
            print('\n---------------------------------------------------------')
            print('Getting the values in each CPASG from Map 04:')

            Join_and_Calc_Fields_CPASG(cpasg_tbl, cmty_plan_id_exst_fld, approved_du_map_04_cpasg_tbl, approved_du_map_04_exst_cpasg_fld, approved_du_map_04_new_fld)
            ...
            arcpy.CalculateField_management(cpasg_tbl, curnt_dev_potentl_map_06_new_fld, expression, 'PYTHON_9.3')
